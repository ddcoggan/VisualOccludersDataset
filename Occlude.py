...
class Occlude(torch.nn.Module):
    ...
    """
    This class is used to augment images with occlusion, and can be used to
    train or evaluate models, or simply to generate occluded versions of an
    image dataset. It can be used in the same way as any torchvision transform,
    and is compatible with the Compose class. The occluder images are loaded
    from disk and a separate transform is applied before they are superimposed
    onto the input image. Usage example is shown in the main function at the
    bottom of this script. Running this will display a series of occluded random
    ...
    args: A SimpleNamespace object containing the dataset configuration.
        This must contain the following attributes:
            ...
            Occlusion: A SimpleNamespace object containing the occlusion
                ...
                Note: if a list is submitted under 'form', 'visibility', or
                color', the occluder for each input image in a batch will be
                configured by sampling randomly from the list. To use the same
                exact occluder across a batch of images, pass a seed (e.g.
                returned by torch.Generator().get_state()) to the Occlude
                object at the start of each batch using the set_seed function.

    preload: set to 'images' if the occluders are stored as image files
        (easier if using custom occluder images). Set to 'tensors' if they are
        grouped in a single tensor in a .pt file (faster loading). The Visual
        Occluders Dataset has both options available.

    mode: 'train' or 'eval', determines whether the occluder transform is random
        or deterministic, respectively.
    """

    def __init__(self, args, preload='tensors', mode='eval'):
        ...
        assert op.isdir(occluder_dir), 'occluder_dir does not exist'
        ...
        directory, allowing mix of L and RGBA modes for silhouette 
        style and textured occluders, respectively"""
        self.occluders = []
        ...
        for occ_dir in occ_dirs:
            ...
            if preload == 'tensors':  # method 2: load single tensor file
    ...
    def set_seed(self, seed):
        ...
        torch.randint or torch.rand below. If the seed is not set,
        self.random_batch behaves exactly like the default generator.
        """
...
if __name__ == "__main__":
    ...
    args = SimpleNamespace(**{
        "Occlusion": {
            "color": "random",
            "form": ["mud_splash", 'natural'],
            "occluder_dir": "/Users/david/Desktop/VisualOccludersDataset",
            "probability": 1,
            "views": [0],
            "visibility": 0.2,
        },
